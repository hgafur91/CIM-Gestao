...
// ── Storage (localStorage for real browser deployment) ────────────────────────
const db = {
...
// ── AI Analysis ───────────────────────────────────────────────────────────────
async function analyzeReport(base64, mimeType, students, apiKey) {
...
Extrai os dados em JSON puro (sem markdown):
{
  ...
  "cim": { "aulas_sem1": número, "aulas_sem2": número, "total_aulas": número, "faltas_sem1": número, "faltas_sem2": número, "total_faltas": número, "materias": "texto" },
  "madrassa": { "sessoes_sem1": número, "sessoes_sem2": número, "total_sessoes": número, "presencas_sem1": número, "presencas_sem2": número, "total_presencas": número, "faltas_sem1": número, "faltas_sem2": número, "total_faltas": número },
  "alunos": [{ "codigo": "código", "nome": "nome", "cim_faltas": número, "madrassa_presencas": número, "madrassa_faltas": número, "madrassa_faltas_justificadas": número, "observacoes": "texto ou null" }],
...
const Badge = ({ children, color = "blue" }) => {
  const map = { blue: [C.bluePale, C.blue], green: [C.greenPale, C.green], amber: [C.amberPale, C.amber], red: [C.redPale, C.red], slate: [C.sand, C.slate] };
...
const Card = ({ children, style, onClick }) => (
  <div onClick={onClick} style={{ background: C.white, borderRadius: 14, border: `1px solid ${C.line}`,
...
export default function App() {
  ...
  function login(id, pass) {
    if (id === "coord" && pass === (db.get("cim:coordpass") || "admin123")) {
  ...
  if (screen === "coord")   return <CoordShell user={user} teachers={teachers} students={students} reports={reports} save={save} apiKey={apiKey} onLogout={logout} />;
  if (screen === "teacher") return <TeacherShell user={user} students={students} reports={reports} save={save} apiKey={apiKey} onLogout={logout} />;
}
...
function LoginScreen({ onLogin }) {
  ...
  function submit() {
    if (!onLogin(id.trim(), pass)) setErr("Credenciais inválidas.");
  }
      ...
      <div style={{ width: "100%", maxWidth: 400, position: "relative" }}>
        <div style={{ textAlign: "center", marginBottom: "2.5rem" }}>
          <div style={{ width: 72, height: 72, background: "rgba(255,255,255,0.1)", borderRadius: 20,
            ...
            marginBottom: "1rem", border: "1px solid rgba(255,255,255,0.15)" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.88rem" }}>Sistema de Gestão de Professores</p>
        </div>
        <div style={{ background: C.white, borderRadius: 18, padding: "2rem", boxShadow: "0 24px 64px rgba(0,0,0,0.3)" }}>
          ...
          {err && <div style={{ color: C.red, fontSize: "0.84rem", marginBottom: "1rem",
            background: C.redPale, borderRadius: 8, padding: "0.6rem 0.9rem",
...
// ── Coordinator Shell ─────────────────────────────────────────────────────────
function CoordShell({ user, teachers, students, reports, save, apiKey, onLogout }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.sand, fontFamily: "'Segoe UI', system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
      <header style={{ background: C.navy, padding: "0 1.5rem", display: "flex",
        alignItems: "center", justifyContent: "space-between", height: 60, flexShrink: 0,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ color: "rgba(255,255,255,0.55)", fontSize: "0.82rem" }}>Coordenador</span>
        </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <aside style={{ width: 220, background: C.white, borderRight: `1px solid ${C.line}`, padding: "1.5rem 0.75rem", flexShrink: 0 }}>
          {nav.map(n => {
            ...
            return (
              <button key={n.id} onClick={() => { setTab(n.id); setDetail(null); }} style={{
                ...
                {n.label}
                {n.badge > 0 && <span style={{ marginLeft: "auto", background: C.amber, color: C.white,
                  borderRadius: 20, padding: "1px 7px", fontSize: "0.7rem", fontWeight: 700 }}>{n.badge}</span>}
              </button>
            );
          })}
        </aside>
        <main style={{ flex: 1, padding: "2rem", overflowY: "auto" }}>
          {detail?.type === "report"  && <ReportDetail report={detail.data} teachers={teachers} reports={reports} onSave={save.reports} onBack={() => setDetail(null)} />}
          {detail?.type === "student" && <StudentDetail student={detail.data} reports={reports} teachers={teachers} onBack={() => setDetail(null)} />}
          {!detail && tab === "overview"  && <CoordOverview teachers={teachers} students={students} reports={reports} onSelectReport={r => setDetail({ type: "report", data: r })} />}
          {!detail && tab === "teachers"  && <TeacherManager teachers={teachers} students={students} onSave={save.teachers} />}
          {!detail && tab === "students"  && <StudentManager students={students} teachers={teachers} onSave={save.students} onSelect={s => setDetail({ type: "student", data: s })} />}
          {!detail && tab === "reports"   && <ReportList reports={reports} teachers={teachers} onSave={save.reports} onSelect={r => setDetail({ type: "report", data: r })} />}
          {!detail && tab === "settings"  && <Settings apiKey={apiKey} onSave={save.apiKey} />}
        </main>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ marginBottom: "2rem" }}>
        <h1 style={T.h1}>Definições</h1>
        <p style={{ ...T.body, marginTop: 4 }}>Configurações do sistema</p>
      </div>
      <Card style={{ maxWidth: 520 }}>
        <h2 style={{ ...T.h2, fontSize: "1rem", marginBottom: "0.5rem" }}>Chave API Anthropic</h2>
        <p style={{ ...T.body, marginBottom: "1rem" }}>
          Necessária para a análise automática dos scans. Obtém a tua chave em{" "}
        ...
        <div style={{ marginBottom: "1rem" }}>
          ...
          <input style={inp} type="password" value={key} placeholder="sk-ant-..."
  ...
  return (
    <div>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ ...T.body, marginTop: 4 }}>Painel de gestão da equipa</p>
      </div>
...
function TeacherManager({ teachers, students, onSave }) {
  ...
  function add() {
    if (!name.trim() || !pass.trim() || !turma.trim()) { setMsg({ text: "Preenche todos os campos.", type: "error" }); return; }
    ...
    <div>
      ...
      <Card style={{ marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: "1rem", marginBottom: "1rem" }}>
          {[["Nome Completo", name, setName, "Ex: Ahmed Mansur"], ["Turma", turma, setTurma, "Ex: Turma A"], ["Palavra-passe", pass, setPass, "Senha de acesso"]].map(([label, val, set, ph]) => (
            <div key={label}>
      ...
      <div style={{ display: "grid", gap: "0.75rem" }}>
        {teachers.map(t => {
          ...
          return (
            <Card key={t.id} style={{ padding: "1rem 1.2rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center" }}>
                    ...
                    <span style={T.small}>· {t.turma} · {n} aluno{n !== 1 ? "s" : ""}</span>
                  </div>
                </div>
              </div>
        ...
        {teachers.length === 0 && <div style={{ textAlign: "center", padding: "3rem", color: C.slateLight }}>
...
function StudentManager({ students, teachers, onSave, onSelect }) {
  ...
  function add() {
    ...
    onSave([...students, { id: `s_${Date.now()}`, name: name.trim(), code: code.trim(),
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: "1.5rem" }}>
        <h2 style={{ ...T.h2, fontSize: "1rem", marginBottom: "1.2rem" }}>Novo Aluno</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: "1rem", marginBottom: "1rem" }}>
          <div>
          ...
          <div>
            <div style={{ ...T.label, marginBottom: 5 }}>Código</div>
            <input style={inp} value={code} placeholder="Ex: ALU001" onChange={e => setCode(e.target.value)} />
          </div>
          <div>
            <div style={{ ...T.label, marginBottom: 5 }}>Professor / Turma</div>
      ...
      <div style={{ position: "relative", marginBottom: "1rem" }}>
        ...
        <input style={{ ...inp, paddingLeft: "2.4rem" }} placeholder="Pesquisar..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      <div style={{ display: "grid", gap: "0.6rem" }}>
        {filtered.map(s => {
          const teacher = teachers.find(t => t.id === s.teacherId);
          return (
            <Card key={s.id} style={{ padding: "0.9rem 1.2rem", display: "flex", alignItems: "center", justifyContent: "space-between" }} onClick={() => onSelect(s)}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <button onClick={e => { e.stopPropagation(); if (window.confirm("Remover?")) onSave(students.filter(x => x.id !== s.id)); }}
        ...
        {filtered.length === 0 && <div style={{ textAlign: "center", padding: "3rem", color: C.slateLight }}>
    ...
    <div>
      ...
      <div style={{ margin: "1.5rem 0" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {teacher && <span style={T.small}>Prof: {teacher.name}</span>}
        </div>
      </div>
      ...
      <h2 style={{ ...T.h2, marginBottom: "1rem" }}>Histórico</h2>
      {sReports.length === 0 ? <p style={T.body}>Sem registos ainda.</p> :
        sReports.map(r => {
          ...
          return (
            <Card key={r.id} style={{ marginBottom: "0.75rem", padding: "1rem 1.2rem" }}>
              ...
              {upd && <div style={T.small}>Faltas CIM: {upd.cim_faltas ?? "—"} · Presenças: {upd.madrassa_presencas ?? "—"} · Faltas: {upd.madrassa_faltas ?? "—"}{upd.observacoes && <> · <i>{upd.observacoes}</i></>}</div>}
            </Card>
          );
        })
      }
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: "0.75rem" }}>
          ...
          return (
            <Card key={r.id} style={{ padding: "1rem 1.2rem", display: "flex",
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                {!r.approved && <Btn variant="success" size="sm" icon="check"
        ...
        {reports.length === 0 && <div style={{ textAlign: "center", padding: "3rem", color: C.slateLight }}>
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", margin: "1.5rem 0", flexWrap: "wrap", gap: "1rem" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          {!report.approved && <Btn variant="success" icon="check"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "1.5rem" }}>
        {[
          { title: "CIM — Estudos Islâmicos", bg: C.blue, rows: [["Aulas Dadas", d.cim?.aulas_sem1, d.cim?.aulas_sem2, d.cim?.total_aulas], ["Faltas Alunos", d.cim?.faltas_sem1, d.cim?.faltas_sem2, d.cim?.total_faltas]], extra: d.cim?.materias ? `Matérias: ${d.cim.materias}` : null },
          { title: "Madrassa — Quran & Duaas", bg: C.navy, rows: [["Sessões Dadas", d.madrassa?.sessoes_sem1, d.madrassa?.sessoes_sem2, d.madrassa?.total_sessoes], ["Presenças", d.madrassa?.presencas_sem1, d.madrassa?.presencas_sem2, d.madrassa?.total_presencas], ["Faltas (A+FJ)", d.madrassa?.faltas_sem1, d.madrassa?.faltas_sem2, d.madrassa?.total_faltas]] },
        ].map(sec => (
          <Card key={sec.title} style={{ padding: 0, overflow: "hidden" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>{sec.rows.map(([label, v1, v2, total]) => <StatRow key={label} label={label} v1={v1} v2={v2} total={total} />)}</tbody>
            </table>
              ...
              <thead>
                <tr style={{ background: C.blueFaint }}>
                  {["Código", "Nome", "Faltas CIM", "Presenças Mad.", "Faltas Mad.", "Observações"].map(h => (
                    <th key={h} style={{ padding: "0.5rem 1rem", textAlign: "left", fontSize: "0.72rem",
                      fontWeight: 700, color: C.slateLight, textTransform: "uppercase", letterSpacing: "0.05em", whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {report.studentUpdates.map((u, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${C.line}`, background: i % 2 === 0 ? C.white : C.blueFaint }}>
                    ...
                    <td style={{ padding: "0.55rem 1rem", fontSize: "0.875rem", fontWeight: 500 }}>{u.nome}</td>
...
// ── Teacher Shell ─────────────────────────────────────────────────────────────
function TeacherShell({ user, students, reports, save, apiKey, onLogout }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.sand, fontFamily: "'Segoe UI', system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
      <header style={{ background: C.navy, padding: "0 1.5rem", display: "flex",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Icon name="mosque" size={22} color={C.blueLight} />
          <span style={{ color: C.white, fontWeight: 700 }}>C.I.M</span>
          <span style={{ color: "rgba(255,255,255,0.25)", margin: "0 4px" }}>|</span>
          <span style={{ color: "rgba(255,255,255,0.55)", fontSize: "0.82rem" }}>{user.name} · {user.turma}</span>
        </div>
        <button onClick={onLogout} style={{ background: "none", border: "none", cursor: "pointer",
          display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,0.5)", fontSize: "0.82rem" }}>
          <Icon name="logout" size={16} color="rgba(255,255,255,0.5)" /> Sair
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <main style={{ flex: 1, padding: "2rem", overflowY: "auto" }}>
          {tab === "submit"   && <SubmitReport user={user} myStudents={myStudents} reports={reports} onSave={save.reports} onUpdateStudents={save.students} students={students} apiKey={apiKey} />}
...
function SubmitReport({ user, myStudents, reports, onSave, onUpdateStudents, students, apiKey }) {
  ...
  async function analyse() {
    if (!file) return;
    if (!apiKey) { setError("Chave API não configurada. O coordenador deve configurar a chave API nas Definições."); return; }
    ...
    try {
      ...
      const data = await analyzeReport(b64, file.type, myStudents, apiKey);
  ...
  async function submit() {
    ...
    const report = {
      id: `r_${user.id}_${Date.now()}`, teacherId: user.id,
      data: result.data, studentUpdates,
      imageBase64: result.b64, imageMime: result.mime,
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: result ? "1fr 1fr" : "1fr", gap: "1.5rem" }}>
        <Card>
          ...
          {error && <div style={{ display: "flex", alignItems: "center", gap: 8, color: C.red,
            fontSize: "0.84rem", marginTop: "0.8rem", background: C.redPale, borderRadius: 8, padding: "0.6rem 0.9rem" }}>
        ...
        {result && (
          <Card>
            ...
            {result.data.alunos?.length > 0 && <div style={{ ...T.small, marginBottom: "1rem" }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: "0.75rem" }}>
        ...
        {myReports.length === 0 && <div style={{ textAlign: "center", padding: "3rem", color: C.slateLight }}>
          <Icon name="report" size={40} color={C.line} />
...
function MyStudents({ myStudents, user }) {
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: "0.6rem" }}>
        {myStudents.map((s, i) => (
          <Card key={s.id} style={{ padding: "0.9rem 1.2rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        {myStudents.length === 0 && <div style={{ textAlign: "center", padding: "3rem", color: C.slateLight }}>
          <Icon name="student" size={40} color={C.line} />
