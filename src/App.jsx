...
const Badge = ({ children, color="blue" }) => {
  const M = { blue:[C.bluePale,C.blue], green:[C.greenPale,C.green], amber:[C.amberPale,C.amber], red:[C.redPale,C.red], slate:[C.sand,C.slate], purple:[C.purplePale,C.purple] };
...
const ProfileHeader = ({ title, subtitle, fields, accentColor, badge, onEdit }) => (
  <div style={{background:`linear-gradient(135deg,${C.navy} 0%,${accentColor||C.blue} 100%)`,borderRadius:20,padding:"2rem",marginBottom:"1.5rem",boxShadow:"0 16px 48px rgba(10,22,40,0.2)",position:"relative",overflow:"hidden"}}>
    ...
    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"1.5rem",flexWrap:"wrap",gap:"1rem"}}>
      ...
      {badge && (
        <div style={{background:"rgba(255,255,255,0.15)",borderRadius:12,padding:"0.6rem 1rem",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)"}}>
          {badge}
        </div>
      )}
    </div>
    ...
    {onEdit && (
      <div style={{marginTop:"1rem"}}>
        <button onClick={onEdit} style={{background:"rgba(255,255,255,0.15)",border:"1px solid rgba(255,255,255,0.3)",color:C.white,borderRadius:8,padding:"0.4rem 0.9rem",cursor:"pointer",fontSize:"0.82rem",fontWeight:600}}>
          Editar Informação
        </button>
      </div>
    )}
...
// ── Schedule table ─────────────────────────────────────────────────────────────
const ScheduleTable = ({ slots, classes }) => {
            ...
            return (
              <tr key={time} style={{background:i%2===0?C.white:C.blueFaint}}>
                ...
                {DAYS.map(day=>{
                  ...
                  const cls=slot&&classes?classes.find(c=>c.id===slot.classId):null;
                    ...
                    <td key={day} style={{padding:"0.5rem",textAlign:"center",borderLeft:`1px solid ${C.line}`}}>
                      {slot?(
                        <div style={{background:color+"18",borderRadius:6,padding:"4px 6px"}}>
                          <div style={{fontWeight:600,fontSize:"0.78rem",color}}>{slot.className||slot.subjectName||"—"}</div>
                          {slot.subjectName&&slot.className&&<div style={{fontSize:"0.68rem",color:C.slate}}>{slot.subjectName}</div>}
                        </div>
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${C.navy} 0%,#112244 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{width:"100%",maxWidth:400}}>
        <div style={{textAlign:"center",marginBottom:"2.5rem"}}>
          <div style={{width:72,height:72,background:"rgba(255,255,255,0.1)",borderRadius:20,display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:"1rem",border:"1px solid rgba(255,255,255,0.15)"}}>
            <Icon name="mosque" size={36} color={C.white} sw={1.4}/>
          </div>
        ...
        <div style={{background:C.white,borderRadius:18,padding:"2rem",boxShadow:"0 24px 64px rgba(0,0,0,0.3)"}}>
          ...
          <div style={{marginTop:"1.2rem",padding:"0.8rem",background:C.sand,borderRadius:8,border:`1px solid ${C.line}`}}>
            <div style={{...T.small,lineHeight:1.8}}><b>Coordenador:</b> coord / admin123<br/><b>Professor:</b> ID / senha<br/><b>Aluno:</b> nº CIM / primeiro nome</div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <Shell user={user} nav={nav} onLogout={onLogout}>
      {({tab,detail,setDetail})=><>
        {detail?.type==="teacher" && <TeacherCard teacher={detail.data} data={data} onSaveTeachers={save.teachers} onBack={()=>setDetail(null)}/>}
...
function CoordOverview({ data, onSelectReport }) {
  ...
  return (
    <div>
      <div style={{marginBottom:"2rem"}}><h1 style={T.h1}>Visão Geral</h1></div>
        ...
        <div style={{display:"grid",gap:"0.75rem"}}>
          {pending.map(r=>{const t=teachers.find(t=>t.id===r.teacherId);return(
            <Card key={r.id} style={{padding:"1rem 1.2rem",display:"flex",alignItems:"center",justifyContent:"space-between"}} onClick={()=>onSelectReport(r)}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}><Badge color="amber">Pendente</Badge><Icon name="eye" size={16} color={C.slateLight}/></div>
            </Card>
          );})}
        </div>
      </>}
...
function LevelManager({ data, onSave }) {
  ...
  return (
    <div>
      <div style={{marginBottom:"2rem"}}><h1 style={T.h1}>Níveis de Professor</h1><p style={{...T.body,marginTop:4}}>Define os níveis, patentes e cores para classificar os professores</p></div>
      <Card style={{marginBottom:"1.5rem"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 90px",gap:"1rem",marginBottom:"1rem"}}>
          <div><div style={{...T.label,marginBottom:5}}>Nome do Nível</div><input style={inp} value={form.name} placeholder="Ex: Júnior, Sénior, Mestre..." onChange={e=>setForm({...form,name:e.target.value})}/></div>
          <div><div style={{...T.label,marginBottom:5}}>Patente / Título</div><input style={inp} value={form.patente} placeholder="Ex: Professor Certificado" onChange={e=>setForm({...form,patente:e.target.value})}/></div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"1rem"}}><Btn icon="plus" onClick={add}>Criar Nível</Btn><Msg {...msg}/></div>
      </Card>
      <div style={{display:"grid",gap:"0.6rem"}}>
        {levels.map(l=>(
          <Card key={l.id} style={{padding:"0.9rem 1.2rem",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:40,height:40,borderRadius:10,background:l.color+"22",border:`2px solid ${l.color}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
                <div style={{width:16,height:16,borderRadius:"50%",background:l.color}}/>
              </div>
...
function TeacherManager({ data, onSave, onSelect }) {
  ...
  function add(){
    ...
    setForm({name:"",password:"",email:"",telefone:"",morada:"",grauAcademico:"",levelId:"",anoAdesao:String(new Date().getFullYear())});
    setShowForm(false);setMsg({text:`Criado! Acesso: ${id} / ${form.password}`,type:"success"});
  }
              ...
              {isEd?(
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                  <div style={{display:"flex",alignItems:"center",gap:14,cursor:"pointer",flex:1,minWidth:0}} onClick={()=>onSelect(t)}>
                    <div style={{width:44,height:44,borderRadius:12,background:level?level.color+"22":C.bluePale,border:level?`2px solid ${level.color}`:undefined,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      <Icon name="users" size={20} color={level?level.color:C.blue}/>
                    </div>
                    <div style={{minWidth:0}}>
                      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={T.h3}>{t.name}</span>
                        {level&&<span style={{background:level.color+"22",color:level.color,borderRadius:20,padding:"1px 8px",fontSize:"0.7rem",fontWeight:700}}>{level.name}</span>}
                      </div>
                      <div style={{display:"flex",gap:8,marginTop:4,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={T.mono}>{t.id}</span>
                        <span style={T.small}>· {myClasses.length} turma{myClasses.length!==1?"s":""} · {n} aluno{n!==1?"s":""}{t.anoAdesao?` · Desde ${t.anoAdesao}`:""}</span>
                      </div>
                    </div>
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <Btn variant="secondary" size="sm" icon="edit" onClick={()=>setEditing({...t})}>Editar</Btn>
                    <Btn variant="danger" size="sm" icon="trash" onClick={()=>{if(window.confirm("Remover?"))onSave(teachers.filter(x=>x.id!==t.id));}}>Remover</Btn>
                  </div>
                </div>
              )}
            </Card>
...
function TeacherCard({ teacher, data, onSaveTeachers, onBack }) {
  ...
  function downloadSchedule(){
    ...
    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Horário - ${teacher.name}</title><style>body{font-family:Arial,sans-serif;padding:20px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:8px;text-align:center}th{background:#0A1628;color:white}tr:nth-child(even){background:#f4f8fe}</style></head><body><h2>Horário — ${teacher.name}</h2>${el.innerHTML}</body></html>`;
    const blob=new Blob([html],{type:"text/html"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download=`horario_${teacher.name.replace(/\s+/g,"_")}.html`;a.click();URL.revokeObjectURL(url);
  }
  ...
  return (
    <div>
      ...
      <ProfileHeader
        ...
        accentColor={level?.color||C.blue}
        badge={level&&(
          <div>
          ...
          return(
            <Card key={cls.id} style={{padding:"1rem 1.2rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:(cls.subjects||[]).length?8:0}}>
                ...
                <div>
                  <div style={{fontWeight:600,color:C.navy}}>{cls.name}<span style={{marginLeft:8,background:color+"22",color,borderRadius:20,padding:"1px 7px",fontSize:"0.68rem",fontWeight:700}}>{isCIM?"CIM":"Madrassa"}</span></div>
      ...
      <div ref={scheduleRef}>
        <ScheduleTable slots={allSlots} classes={classes}/>
      </div>
    </div>
  );
}

// ── Student Card (Coordinator view) ──────────────────────────────────────────
function StudentCard({ student, data, onSaveStudents, onBack }) {
  ...
  function saveEdit(){
    onSaveStudents(data.students.map(s=>s.id===student.id?{...s,...form}:s));
    setEditing(false);
  }

  const INFO_FIELDS=[["Data de Nascimento","dataNascimento","DD/MM/AAAA"],["Morada","morada","Rua, cidade"],["Contacto dos Pais","contactoPais","+351 9xx xxx xxx"],["Nome do Encarregado","nomePai","Nome completo"],["Ano de Adesão","anoAdesao","Ex: 2023"]];

  // Build schedule slots for student's class
    ...
    <div>
        ...
        <Card style={{marginBottom:"1.5rem"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"1rem",marginBottom:"1rem"}}>
            {INFO_FIELDS.map(([label,field,ph])=>(
              <div key={field}><div style={{...T.label,marginBottom:5}}>{label}</div><input style={inp} value={form[field]||""} placeholder={ph} onChange={e=>setForm({...form,[field]:e.target.value})}/></div>
            ))}
          </div>
      ...
      {subjects.length>0&&<>
        ...
        {subjects.map(subj=>{
          ...
          const t1=trimAvg(g,1),t2=trimAvg(g,2),t3=trimAvg(g,3),mF=avg(t1,t2,t3);
          return(
                ...
                {[1,2,3].map(t=>(
                  <div key={t} style={{padding:"0.75rem",borderRight:`1px solid ${C.line}`,textAlign:"center"}}>
                    ...
                    <div style={{display:"flex",gap:6,justifyContent:"center",marginBottom:4}}>
                      {["1as","2as","at"].map(f=><div key={f} style={{textAlign:"center"}}><div style={{...T.small,fontSize:"0.62rem"}}>{f.toUpperCase()}</div><div style={{fontWeight:700}}><GradeCell val={g[`t${t}_${f}`]}/></div></div>)}
                    </div>
      ...
      {/* Horário */}
      {cls&&<><h2 style={{...T.h2,margin:"1.5rem 0 1rem"}}>Horário da Turma</h2><ScheduleTable slots={slots}/></>}
    </div>
  );
}
...
function ClassManager({ data, onSaveClasses }) {
  ...
  const [type,setType]=useState("cim");
  const [teacherId,setTeacherId]=useState("");
  const [msg,setMsg]=useState({text:"",type:""});
  const [openId,setOpenId]=useState(null);
  const [openTab,setOpenTab]=useState("subjects");
  ...
  function addClass(){
    ...
    onSaveClasses([...classes,{id,name:name.trim(),year,type,teacherId,subjects:[],schedule:[],createdAt:new Date().toISOString()}]);
  ...
  function addSlot(cid,ctype){
    ...
    onSaveClasses(classes.map(c=>c.id===cid?{...c,schedule:[...(c.schedule||[]),{id:`sl_${Date.now()}`,...slot}]}:c));
    setSlot(emptySlot);
  }
    ...
    <div>
      ...
      <Card style={{marginBottom:"1.5rem"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:"1rem",marginBottom:"1rem"}}>
          ...
          <div><div style={{...T.label,marginBottom:5}}>Professor</div>
                  ...
                  {isOpen&&openTab==="subjects"&&isCIM&&(
                    <div style={{marginTop:"1rem",paddingTop:"1rem",borderTop:`1px solid ${C.line}`}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:"0.75rem"}}>
                        {subjects.map(s=>(
                          <div key={s.id} style={{display:"flex",alignItems:"center",gap:5,background:C.bluePale,borderRadius:20,padding:"0.25rem 0.75rem"}}>
                            <span style={{fontSize:"0.82rem",color:C.blue,fontWeight:600}}>{s.name}</span>
                            <button style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex"}} onClick={()=>removeSubject(cls.id,s.id)}><Icon name="trash" size={13} color={C.slateLight}/></button>
                          </div>
                        ))}
                        {subjects.length===0&&<span style={T.small}>Sem disciplinas ainda.</span>}
                      </div>
                      <div style={{display:"flex",gap:8}}>
                        <input style={{...inp,flex:1}} value={subjInput} placeholder="Nome da disciplina..." onChange={e=>setSubjInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addSubject(cls.id)}/>
                            ...
                            <div key={day} style={{display:"flex",gap:8,alignItems:"flex-start",flexWrap:"wrap"}}>
                              ...
                              <div style={{display:"flex",gap:6,flexWrap:"wrap",flex:1}}>
                                {daySlots.map(s=>{
                                  ...
                                  const slotT=teachers.find(t=>t.id===s.slotTeacherId);
                                  return(
  ...
  return (
    <div>
      ...
      <div style={{position:"relative",marginBottom:"1rem"}}>
        <div style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)"}}><Icon name="search" size={16} color={C.slateLight}/></div>
        <input style={{...inp,paddingLeft:"2.4rem"}} placeholder="Pesquisar por nome ou código CIM..." value={search} onChange={e=>setSearch(e.target.value)}/>
      </div>
      <div style={{display:"grid",gap:"0.6rem"}}>
        {filtered.map(s=>{
          ...
          return(
            <Card key={s.id} style={{padding:"0.9rem 1.2rem"}}>
              ...
              {isTr&&(
                <div style={{marginTop:"0.75rem",paddingTop:"0.75rem",borderTop:`1px solid ${C.line}`,display:"flex",gap:8,alignItems:"flex-end"}}>
                  <div style={{flex:1}}>
                    ...
                    <select style={{...inp}} value={newClassId} onChange={e=>setNewClassId(e.target.value)}>
                      ...
                      {classes.filter(c=>c.id!==s.classId).map(c=>{const t=teachers.find(t=>t.id===c.teacherId);return<option key={c.id} value={c.id}>{c.name} {t?`— ${t.name}`:""}</option>;})}
                    </select>
                  </div>
        ...
        {filtered.length===0&&<div style={{textAlign:"center",padding:"3rem",color:C.slateLight}}><Icon name="student" size={40} color={C.line}/><p style={{marginTop:"1rem"}}>Nenhum aluno encontrado</p></div>}
      </div>
    </div>
  );
}
...
function GradeManager({ data, onSave, filterTeacherId }) {
  ...
  const myClasses=filterTeacherId?classes.filter(c=>c.teacherId===filterTeacherId):classes;
  ...
  function updGrade(sid,subjId,field,val){
    ...
    <div>
      ...
      {selClass&&<>
        <div style={{display:"flex",gap:"0.5rem",flexWrap:"wrap",marginBottom:"1.2rem"}}>
          {clsSubjects.map(s=>(
            <button key={s.id} onClick={()=>{setSelSubj(s);setEditing({});}} style={{padding:"0.4rem 0.9rem",borderRadius:20,cursor:"pointer",fontFamily:"inherit",fontSize:"0.82rem",fontWeight:600,border:`1.5px solid ${selSubj?.id===s.id?C.blue:C.line}`,background:selSubj?.id===s.id?C.blue:C.white,color:selSubj?.id===s.id?C.white:C.slate}}>{s.name}</button>
          ))}
              ...
              <table style={{width:"100%",borderCollapse:"collapse",minWidth:800}}>
                <thead>
                  ...
                  <tr style={{background:C.blueFaint}}>
                    ...
                    {TRIMS.map(({t})=><>{FIELDS.map(f=><th key={f.key} style={{...thS,fontSize:"0.66rem"}}>{f.label}</th>)}<th key={`m${t}`} style={{...thS,fontSize:"0.66rem",color:C.blue}}>Méd.</th></>)}
                    <th style={thS}/><th style={thS}/>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return(
                      <tr key={s.id} style={{background:i%2===0?C.white:C.blueFaint,borderBottom:`1px solid ${C.line}`}}>
                        ...
                        <td style={{textAlign:"center",padding:"0.3rem"}}>
                          {canExam?<GradeInput value={g.exame_final} onChange={v=>updGrade(s.id,selSubj.id,"exame_final",v)}/>:<span style={{...T.small,fontSize:"0.68rem"}}>Méd. &lt; 12</span>}
                        </td>
                      </tr>
                    );
                  })}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1.5rem",marginBottom:"1.5rem"}}>
        {[
          ...
          <Card key={sec.title} style={{padding:0,overflow:"hidden"}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:C.blueFaint}}>{["","Sem. 1","Sem. 2","Total"].map(h=><th key={h} style={{padding:"0.5rem 1rem",textAlign:h?"center":"left",fontSize:"0.72rem",fontWeight:700,color:C.slateLight,textTransform:"uppercase"}}>{h}</th>)}</tr></thead>
...
function TeacherSchedulePage({ user, data }) {
  ...
  return (
    <div>
      <div style={{marginBottom:"2rem"}}><h1 style={T.h1}>O Meu Horário</h1><p style={{...T.body,marginTop:4}}>Horário semanal das tuas turmas</p></div>
...
function TeacherStudents({ user, data, onSave, myClasses }) {
  ...
  function add(){
    ...
    onSave([...students,{id:`s_${Date.now()}`,name:name.trim(),code,classId:selClass.id,teacherId:user.id,createdAt:new Date().toISOString(),grades:{},level:isMadrassa?"1ª Parte":null,licaoApresentada:isMadrassa?"":null,licaoPorApresentar:isMadrassa?"":null,stats:{cim_faltas:0,madrassa_presencas:0,madrassa_faltas:0,madrassa_fj:0}}]);
    setName("");setMsg({text:`${name} adicionado — ${code}`,type:"success"});
      ...
      {selClass?(
        <>
          <Card style={{marginBottom:"1.5rem"}}>
            ...
            <Msg {...msg}/>
            <p style={{...T.small,marginTop:"0.5rem"}}>Número CIM atribuído automaticamente — CIM0001, CIM0002...{isMadrassa?" · Define nível e lições após adicionar.":""}</p>
          </Card>
          <div style={{display:"grid",gap:"0.6rem"}}>
            {clsStudents.map((s,i)=>{
                  ...
                  {isEd?(
                    <div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0.75rem",marginBottom:"0.75rem"}}>
                        ...
                        <div><div style={{...T.label,marginBottom:4}}>Lição Apresentada</div><input style={inp} value={editForm.licaoApresentada||""} placeholder="Ex: Surah Al-Fatiha" onChange={e=>setEditForm({...editForm,licaoApresentada:e.target.value})}/></div>
                        <div><div style={{...T.label,marginBottom:4}}>Lição por Apresentar</div><input style={inp} value={editForm.licaoPorApresentar||""} placeholder="Ex: Surah Al-Baqarah" onChange={e=>setEditForm({...editForm,licaoPorApresentar:e.target.value})}/></div>
                      </div>
        ...
        {myReports.map(r=>(
          <Card key={r.id} style={{padding:"1rem 1.2rem",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <div><div style={T.h3}>{r.data?.semanas}</div><div style={T.small}>{r.data?.periodo} · {new Date(r.createdAt).toLocaleDateString("pt-PT")}</div></div>
            </div>
...
function SubmitReport({ user, myStudents, data, save }) {
  ...
  function submit(){
    ...
    const report={id:`r_${user.id}_${Date.now()}`,teacherId:user.id,data:result.data,studentUpdates:su,imageBase64:result.b64,imageMime:result.mime,approved:false,createdAt:new Date().toISOString()};
  ...
  return(
    <div>
      <div style={{marginBottom:"2rem"}}><h1 style={T.h1}>Submeter Relatório Quinzenal</h1><p style={{...T.body,marginTop:4}}>Fotografa a folha de entrega e faz upload abaixo.</p></div>
  ...
  return(
    <Shell user={user} nav={nav} onLogout={onLogout}>
      {({tab})=><>
        ...
        {tab==="grades" && <MyStudentGrades user={user} data={data}/>}
      </>}
    </Shell>
  );
}
    ...
    <div style={{maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.navy} 0%,${C.blue} 100%)`,borderRadius:20,padding:"2rem",boxShadow:"0 16px 48px rgba(10,22,40,0.25)",position:"relative",overflow:"hidden",marginBottom:"1.5rem"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:"1.8rem"}}>
          ...
          <div><div style={{color:C.white,fontWeight:800,fontSize:"1.1rem"}}>C.I.M</div><div style={{color:"rgba(255,255,255,0.5)",fontSize:"0.72rem"}}>Centro Islâmico</div></div>
      ...
      {subjects.map(subj=>{
        ...
        return(
          <Card key={subj.id} style={{marginBottom:"1rem",overflow:"hidden",padding:0}}>
            <div style={{background:C.navy,padding:"0.75rem 1.2rem",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{color:C.white,fontWeight:700}}>{subj.name}</span>
                    ...
                    <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:8}}>
                      {["1as","2as","at"].map(f=>(
                        <div key={f} style={{textAlign:"center"}}>
                          ...
                          <div style={{fontWeight:700,fontSize:"0.95rem",color:g[`t${t}_${f}`]!=null?gradeColor(g[`t${t}_${f}`]):C.slateLight}}>{g[`t${t}_${f}`]??<span style={{color:C.slateLight}}>—</span>}</div>
                        </div>
                      ))}
                    </div>
